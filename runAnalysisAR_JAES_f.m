% AR model based click-detection test
% Authors: Václav Vencovský, Marek Semanský, Frantisek Rund
% FEE CTU 2021
% MATLAB Version: 9.4.0.813654 (R2018a)


function [Ap,dprime, CD, FA, elapsed_time] = runAnalysisAR_JAES_f(threshold_gain, LengthSeg, Order, sig_list, listen_th)
%clear, close all, %clc
% addpath('fce_AR');  % add path to folder with AR model click detection functions
...
Threshold = threshold_gain; %15; %Detection threshold gain K !
...
disp(['stim: ' num2str(kstim(k))])
    ...
    if ismember(k,idxHIT) %AR model
          
          clickAR(kstim(k)) = ARmodel_mm(y, Order, Threshold, LengthSeg,idxLup,idxTol,1);%click perceived
    else
          
          clickAR(kstim(k)) = ARmodel_mm(y, Order, Threshold, LengthSeg,idxLup,idxTol,0);%click not perceived
    end    
end
